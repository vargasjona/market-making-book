...
function Message({
	...
	return (
		// biome-ignore lint/a11y/noStaticElementInteractions: stops clicks inside a message from closing the dialog
		// biome-ignore lint/a11y/noNoninteractiveElementInteractions: stops clicks inside a message from closing the dialog
		// biome-ignore lint/a11y/useKeyWithClickEvents: click-swallowing only, no action to mirror on keyboard
        ...
        @keyframes ask-ai-close {
          ...
          to {
			...
			<Presence present={open}>
				{/* biome-ignore lint/a11y/noStaticElementInteractions: backdrop click-to-dismiss; Esc handled by the dialog */}
				{/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: backdrop click-to-dismiss; Esc handled by the dialog */}
				{/* biome-ignore lint/a11y/useKeyWithClickEvents: backdrop click-to-dismiss; Esc handled by the dialog */}
				<div
					className={cn(
		...
		>
			{messages.length === 0 ? (
				<div className="flex size-full flex-col items-center justify-center gap-2 text-center text-fd-muted-foreground/80 text-sm">
					...
					{/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: stops clicks from closing the dialog */}
					{/* biome-ignore lint/a11y/useKeyWithClickEvents: click-swallowing only, no action to mirror on keyboard */}
...
export function useAISearchContext() {
	...
	if (!ctx) {
		throw new Error("AI search components must be used within <AISearch>");
	}
	return ctx;
}
